...
class PreparationScene extends Scene {
    ...
    draggedShip = null //свойство, которое ориентируется на перетаскиваемый корабль
    draggedOffsetX = 0;
    draggedOffsetY = 0;
    ...
    update () {
        const { mouse, player} = this.app; // const mouse = this.app.mouse / const player = this.app.player
        ...
        if (!this.draggedShip && mouse.left && !mouse.pLeft) {
         ...
         if (mouse.left && this.draggedShip) {
            ...
            this.draggedShip.div.style.left = `${x + 3}px`;
            this.draggedShip.div.style.top = `${y + 3}px`;
         }
    ...
    startComputer (level) {
        ...
        if (level === 'simple') {
            ...
            untouchables = geRandomtSeveral(withoutShipItems, 20);
        } else if (level === 'hard') {
            untouchables = geRandomtSeveral(withoutShipItems, 40);
        }
